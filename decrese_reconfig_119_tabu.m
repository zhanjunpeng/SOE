...
addpath('./code')
ver_num_reconfig_dec = 3
%% basic setting
tic
fprintf('decrease_reconfig_119_tabu.m \n')
warning('off')
addpath(pathdef)
mpopt = mpoption;
...
fprintf('case119_tabu: original loop network''s loss is %.5f \n\n', loss0)
...
fprintf('case119_tabu: radial network obtained by my core algorithm''s loss is %.5f \n\n', loss0_dec)
...
%     [ending_bus'; Vm(ending_bus)]; 
    ...
    %     Branch_loss_record.tabu_o1c1_dec(iter,1).Branch = Branch_loss.Branch_o1c1_dec; 
        ...
        fprintf('case119_tabu: minimum loss obtained after ''one open and one close'': %.5f\n', ...
            min_loss_o1c1);

        min_loss_combine_two_o1c1 = 1e9;
        fprintf('case119_tabu: loss obtained after combine two ''one open and one close'': \n')
        ...
        fprintf('case119_tabu: minimum loss obtained after combine two ''one open and one close'': %.5f \n', ...
    ...
    if flag_2o2c == 1
        ...
        fprintf('case119_tabu: minimum loss obtained after ''two open and two close'': %.5f\n', ...
            min_loss_o2c2);

        min_loss_combine_two_o2c2 = 1e9;
        fprintf('case119_tabu: loss obtained after combine two ''two open and two close'': \n')
        ...
        fprintf('case119_tabu: minimum loss obtained after combine two ''two open and two close'': %.5f \n', ...
...
%     file_name = ['case119_yij_Branch_', num2str(idx_force_open(iter)), '.mat'];
%     save(file_name, 'yij_dec', 'Branch', 'lossi');
    file_name = ['id', num2str(ver_num_reconfig_dec), ...
        '_case119_yij_Branch', '.mat'];
...
fprintf('case119_tabu: losses obtained after applying tabu strategy: \n') % 0.28343  zjp 2018-1-18
fprintf('%.5f \n', loss_tabu)
fprintf('----- min: %.5f -----\n', min(loss_tabu))
...
%% show my id 177 result, 9% better than franco paper
idx_177 = [  23    25    34    39    43    50    58    63    71 ...
    73    75   109   121   129   130];
selected_brch = [1:132];
selected_brch(idx_177) = [];

show_biograph_not_sorted(Branch0(selected_brch, [1 2]), substation_node, show_biograph1);
