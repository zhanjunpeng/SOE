function [loss_after_switch, Branch_after_switch] = find_loss_change_of_combine_two_o1c1( ...
...
% rows2 = zone_involved{combi(2)};
    ...
    for i=1:length(rows1)
        for j = 1:length(rows2)
            close_connection = record_o1c1_loss_dec([rows1(i), rows2(j)], [3 4]);
            open_connection = record_o1c1_loss_dec([rows1(i), rows2(j)], [5 6]);
