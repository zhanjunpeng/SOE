function [loss_after_switch, Branch_after_switch] = find_loss_change_of_combine_four_o1c1( ...
...
% rows2 = zone_involved{combi(2)};
    ...
    rows4 = find(type_id == combi(4));
    loss_after_switch = [];
    cnt1 = 0;
    for i=1:length(rows1)
        for j = 1:length(rows2)
            for k = 1:length(rows3)
                for kk = 1:length(rows4)
                    close_connection = record_o1c1_loss_dec([rows1(i), rows2(j), rows3(k), rows4(kk)], [3 4]);
                    open_connection = record_o1c1_loss_dec([rows1(i), rows2(j), rows3(k), rows4(kk)], [5 6]);
